...
just the column that is stored in the index, we’re still getting the same benefit. 
That is because the indexed column’s actual value is stored in the index, therefore 
MySQL will use the index value instead of reading the record. However, if we start retrieving columns 
that aren’t part of the index, this is where we want the actual records stored in order, 
...
-- Normal Index
If it’s not primary or unique, it doesn’t constrain values inserted into the table, 
...
Think of it as (essentially) creating an “index” for each “word” in the specified column. 
